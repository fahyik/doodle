...
export function parse_accept_language(input: string, supported: string[]) {
  const inputAsArray: string[] = input.split(",").map((s) => s.trim());

  const result: string[] = [];

  for (const inputLang of inputAsArray) {
    // generic lang case
    if (inputLang === "*") {
      for (const supp of supported) {
        if (!result.includes(supp)) {
          result.push(supp);
        }
      }
      continue;
    ...
    // check input string, if it's two char
    if (inputLang.length === 2) {
      for (const supp of supported) {
        if (inputLang === supp.substring(0, 2)) {
          if (!result.includes(supp)) {
            result.push(supp);
          }
        }
    ...
    // specific lang case
    if (supported.includes(inputLang)) {
      if (!result.includes(inputLang)) {
        result.push(inputLang);
      }
    }
  }
